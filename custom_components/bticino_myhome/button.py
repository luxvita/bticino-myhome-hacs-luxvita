...
class DisableCommandButtonEntity(ButtonEntity, MyHOMEEntity):
    ...
    async def async_added_to_hass(self):
        ...
        self._hass.data[DOMAIN][self._gateway_handler.mac][CONF_PLATFORMS][
            self._platform
        ][self._device_id].setdefault(CONF_ENTITIES, {})["disable"] = self

    async def async_will_remove_from_hass(self):
        """When entity is removed from hass."""
        _entities = self._hass.data[DOMAIN][self._gateway_handler.mac][CONF_PLATFORMS][
            self._platform
        ][self._device_id].get(CONF_ENTITIES, {})
        if "disable" in _entities:
            del _entities["disable"]
    ...
    def handle_event(self, message) -> None:
        """Buttons are command-only entities; bus events carry no state to apply."""
        return
...
class EnableCommandButtonEntity(ButtonEntity, MyHOMEEntity):
    ...
    async def async_added_to_hass(self):
        ...
        self._hass.data[DOMAIN][self._gateway_handler.mac][CONF_PLATFORMS][
            self._platform
        ][self._device_id].setdefault(CONF_ENTITIES, {})["enable"] = self
    ...
    async def async_will_remove_from_hass(self):
        ...
        if "enable" in _entities:
            del _entities["enable"]
