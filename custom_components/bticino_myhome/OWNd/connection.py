...
class OWNGateway:
    def __init__(self, discovery_info: dict):
        ...
        def _coerce_to_str(value):
            """UPnP/SSDP discovery info can hand back a list for fields that
            are supposed to be a single string. Home Assistant's device
            registry rejects non-string values for manufacturer/model/
            sw_version, so normalize defensively wherever discovery_info
            feeds one of those fields."""
        ...
        self.manufacturer = _coerce_to_str(_manufacturer)
        ...
        self.model_name = (
            _coerce_to_str(discovery_info["modelName"])
            if "modelName" in discovery_info
            else "Unknown model"
        )
        self.model_number = (
            _coerce_to_str(discovery_info["modelNumber"])
            if "modelNumber" in discovery_info
            else None
        )
        ...
        # )
        self.serial_number = (
            _coerce_to_str(discovery_info["serialNumber"])
            if "serialNumber" in discovery_info
            else None
        )
