...
class MyHOMEGatewayHandler:
    ...
    async def sending_loop(self, worker_id: int):
        ...
        while not self._terminate_sender:
            ...
            if command_session is not None and last_command_sent_at is not None:
                idle_seconds = time.monotonic() - last_command_sent_at
                if idle_seconds >= COMMAND_SESSION_IDLE_TIMEOUT:
                    LOGGER.debug(
                        "%s Command session idle for %.0fs, recycling before use.",
                        self.log_id,
                        idle_seconds,
                    )
                    try:
                        ...
                        LOGGER.debug(
                            "%s Failed to close idle command session: %s",
                            self.log_id,
                            err,
                        )
                    command_session = None
            ...
            try:
                ...
                # Pace outgoing commands. Sending large bursts (e.g. a full
                # re-poll of every entity after an event session reconnect)
                # back-to-back can outrun what the gateway can relay onto the
                # physical bus, causing it to NACK every queued message.
                if not retry_task and not cancel_requested and not self._terminate_sender:
                    await asyncio.sleep(SEND_THROTTLE_DELAY)
    ...
    async def _repoll_all_entities(self) -> None:
        ...
        try:
            ...
            for platform, entities in platforms.items():
                if platform == BUTTON:
                    continue
                for device_id, device_data in entities.items():
                    if platform == CLIMATE:
                        where = device_data.get(CONF_ZONE, device_id)
                    else:
                        where = device_data.get(CONF_WHERE, device_id)
                    try:
                        if platform == CLIMATE:
                            await self.send_status_request(OWNHeatingCommand.status(where))
                        elif platform == LIGHT:
